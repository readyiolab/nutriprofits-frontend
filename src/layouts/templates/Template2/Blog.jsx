...
const Template2Blog = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
      <section className="relative bg-gradient-to-br from-blue-600 via-indigo-700 to-purple-800 py-16 md:py-24 overflow-hidden">
        <div className="max-w-6xl mx-auto px-4 text-center relative z-10">
          <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-md rounded-full px-4 py-2 mb-6 border border-white/20">
            <Sparkles className="w-4 h-4 text-yellow-300" />
            <span className="text-white/90 text-sm font-medium">Blog</span>
          </div>
          <h1 className="text-3xl md:text-5xl font-bold text-white mb-4">Latest Articles & Stories</h1>
          <p className="text-white/70 text-base md:text-lg max-w-2xl mx-auto mb-8">Explore insightful articles, guides, and stories</p>
          <div className="max-w-md mx-auto relative">
            ...
            <input type="text" placeholder="Search articles..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-11 pr-4 py-3 rounded-full bg-white/95 text-gray-800 placeholder-gray-400 text-sm focus:outline-none focus:ring-2 focus:ring-purple-400 shadow-lg" />
          </div>
        </div>
      </section>
              ...
              <div className="grid md:grid-cols-2 gap-0">
                ...
                <div className="p-6 md:p-8 flex flex-col justify-center">
                  <div className="inline-flex items-center gap-2 mb-3">
                    <span className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-xs font-semibold px-3 py-1 rounded-full">Featured</span>
                  ...
                  <h2 className="text-xl md:text-2xl font-bold text-gray-900 mb-3 group-hover:text-blue-600 transition-colors">{filteredBlogs[0].title}</h2>
                  ...
                  <div className="flex items-center justify-between mt-auto">
                    ...
                    <span className="flex items-center gap-1 text-blue-600 text-sm font-medium group-hover:gap-2 transition-all">Read More <ArrowRight className="w-4 h-4" /></span>
                  </div>
                </div>
              </div>
            </Link>
          ...
          {(searchTerm ? filteredBlogs : filteredBlogs.slice(1)).map((blog) => (
            <article key={blog.blog_id}>
              <Link to={`/template/${templateId}/blog/${blog.slug}`}
                className="group block bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all border border-gray-100 h-full">
                ...
                <div className="p-5">
                  ...
                  <h3 className="text-base font-semibold text-gray-900 mb-2 group-hover:text-blue-600 transition-colors line-clamp-2">{blog.title}</h3>
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-gray-50">
                    ...
                    <span className="flex items-center gap-1 text-blue-600 text-xs font-medium group-hover:gap-2 transition-all">Read <ArrowRight className="w-3 h-3" /></span>
                  </div>
                </div>
              </Link>
            </article>
