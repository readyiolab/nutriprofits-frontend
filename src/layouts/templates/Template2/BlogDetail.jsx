...
const Template2BlogDetail = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
      <div className="bg-gradient-to-r from-blue-600 via-indigo-700 to-purple-800 py-4">
      ...
      <header className="bg-gradient-to-br from-blue-600 via-indigo-700 to-purple-800 pb-12 pt-8">
        <div className="max-w-4xl mx-auto px-4">
          <Link to={`/template/${templateId}/blog`} className="inline-flex items-center gap-2 text-white/70 hover:text-white text-sm mb-6 transition"><ArrowLeft className="w-4 h-4" /> All Articles</Link>
      ...
      <article className="max-w-4xl mx-auto px-4 py-10">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-10">
          <div className="prose prose-base md:prose-lg max-w-none prose-headings:text-gray-900 prose-p:text-gray-700 prose-a:text-blue-600 prose-blockquote:border-blue-500"
            dangerouslySetInnerHTML={{ __html: blog.content }} />
        </div>
        <div className="mt-8 bg-white rounded-xl p-6 border border-gray-100 flex items-center gap-4">
          <div className="w-12 h-12 rounded-full bg-gradient-to-br from-blue-600 to-indigo-700 flex items-center justify-center text-white font-bold text-lg">{blog.author.charAt(0)}</div>
            ...
            {relatedPosts.map((post) => (
              <Link key={post.blog_id} to={`/template/${templateId}/blog/${post.slug}`}
                ...
                <div className="p-4">
                  ...
                  <h3 className="text-sm font-semibold text-gray-900 group-hover:text-blue-600 transition-colors line-clamp-2">{post.title}</h3>
                </div>
              </Link>
            ))}
          </div>
